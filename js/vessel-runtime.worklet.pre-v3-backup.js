...
class VesselRuntime extends AudioWorkletProcessor {
  ...
  constructor(options) {
    ...
    this.subLp = [0, 0, 0];
    this.pulseLp = [0, 0, 0];
    // Optional pure-sub oscillator phase (firing-locked mass)
    this.phaseSub = 0;
  ...
  // ─── BUS 1: Sub & Body (restricted below ~300 Hz) ─────────────────────
  _processSubBus(partialSub, rpm, loadEff, lope, bodyLift) {
    // Pure low-end mass: crank-locked sine (spec baseFreq ≈ rpm/60)
    const baseFreq = (rpm / 120) * 2;
    this.phaseSub += baseFreq / sampleRate;
    if (this.phaseSub >= 1) this.phaseSub -= 1;
    const pureSub = Math.sin(this.phaseSub * TWO_PI) * (0.55 + loadEff * 0.35);
    // DNA low partials already bucketed into partialSub
    const raw = (pureSub * 0.85 + partialSub * 1.15) * (0.9 + loadEff * 0.25) * lope * bodyLift;
    return this._busLPF(this.subLp, raw, this._subA);
  }
  ...
  process(_inputs, outputs, params) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const subSignal = this._processSubBus(partialSub, rpm, loadEff, lope, bodyLift);
      ...
      out[i] = this._tanhSoftClip(scaled, tipDriveBoost + overDriveBoost);
    }
    return true;
  }
}
