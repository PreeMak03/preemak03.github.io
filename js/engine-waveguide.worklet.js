...
class EngineWaveguideProcessor extends AudioWorkletProcessor {
  ...
  constructor(options) {
    ...
    this.eng = new CompactWaveguideEngine(Object.assign({ enabled: true }, o), sampleRate);
    this.port.onmessage = (e) => {
      if (!e.data) return;
      if (e.data.reset) this.eng.reset();
      if (e.data.configure) {
        this.eng.configure(Object.assign({}, e.data.configure, { enabled: true }));
        this.eng.reset();
      }
    };
  }
  process(_inputs, outputs, params) {
    ...
    const mixG = params.mix[0];
    for (let i = 0; i < out.length; i++) {
      const r = this.eng.process(rpm, load);
      out[i] = r.mix * mixG;
    }
    return true;
  }
}
registerProcessor('engine-waveguide', EngineWaveguideProcessor);
