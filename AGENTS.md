...
## LAW 8 — วัดสิ่งที่ "ไม่ได้" กำลังแก้

**ห้ามแก้ค่าที่คุมหลายพฤติกรรม แล้ววัดผลแค่พฤติกรรมที่กำลังแก้**

ความพลาดในโปรเจกต์นี้ไม่เคยเป็นเรื่องที่คิดไม่ถึง — เป็นเรื่องที่**ไม่ได้ไปดู**
พอเจ้าของทักมาว่า "รอบไม่หายใจ" / "คันเร่งไวเกิน" ก็บอกสาเหตุได้ทันทีทุกครั้ง
แปลว่าความรู้มีอยู่แล้ว แต่ตอนแก้ไม่มีอะไรบังคับให้ไปดู

**เกิดขึ้นจริงแล้ว (CRANK, ส.ค. 2026):** ห้าครั้ง รูปแบบเดียวกันหมด
สามในห้าครั้งเจ้าของเป็นคนเจอ ไม่ใช่การวัด — ดูตาราง `docs/FANOUT.md`

**กฎบังคับ:**

1. **เปิด `docs/FANOUT.md` ก่อนแก้ค่าใน audio path**
   ถ้าค่านั้นอยู่ในตาราง = ต้องวัดทุกช่องในคอลัมน์ "พังได้" ไม่ใช่แค่ช่องที่กำลังแก้
2. **หนึ่งแพทช์ = หนึ่งการเปลี่ยนแปลง** เมื่อแตะ audio path
   ครั้งที่ยกไป 4 อย่างพร้อมกันแล้วเสียงแย่ลง **ไม่มีใครบอกได้ว่าตัวไหนผิด** ต้องถอยทั้งชุด
   ของที่ถอยทั้งชุดคือของที่เสียเวลาเปล่าทั้งชุด
3. **รัน `offlineBench.compare()` ก่อนขึ้น** เก็บตัวเลขทั้งสองชุดไว้ในข้อความ commit
   `vessel/tools/offline-bench.js` เรนเดอร์กราฟเสียง**ตัวจริง**ผ่าน OfflineAudioContext
   ไม่ต้องมีรถ ไม่ต้องมีคนขับ ไม่พึ่ง rAF (ซึ่งค้างเมื่อแท็บถูกซ่อน) และเร็วกว่าเวลาจริง
   `bench2.runAll()` ยังใช้ได้ แต่ต้องเปิดแท็บไว้หน้าสุดตลอด — ใช้เป็นตัวเสริม
   ถ้ารันไม่ได้ (ไม่มีเบราว์เซอร์ / ไม่มีเครื่อง) → **บอกเจ้าของว่ายังไม่ได้วัด อย่าเงียบ**
4. **เจ้าของทักเมื่อไหร่ = การวัดพลาด ไม่ใช่แค่โค้ดพลาด**
   แก้โค้ดแล้วต้องถามต่อว่า "ทำไม bench ไม่จับ" แล้วเพิ่มตัวชี้วัดนั้นเข้าไป
   `LIMITS` ใน `crank-bench.js` โตขึ้นได้ตลอด ห้ามปล่อยให้อาการเดิมหลุดซ้ำ
5. **ค่าที่ผู้ใช้หมุนเองต้องวัดที่ค่าสุดขอบ** ไม่ใช่แค่ค่ากลาง
   จูนที่ bass 50 แล้วผู้ใช้หมุนไป 100 = อาการที่เราไม่เคยเห็น (เกิดขึ้นจริงมาแล้ว)
6. **ห้ามเชื่อผลต่างที่ยังไม่รู้พื้นสัญญาณรบกวน**
   เครื่องยนต์หายใจด้วย `Math.random()` (wander / idle hunt / pops) รันโค้ด**เดิม**
   สี่รอบยังได้ต่างกัน 1.3 dB ที่ idle `compare()` จึงเรนเดอร์ซ้ำแล้วตัดสินให้เอง —
   **อ่านช่อง `verdict` ไม่ใช่ช่อง `deltaDb`** ครั้งแรกที่ใช้ bench นี้ มันรายงาน
   "แย่ลง 0.7 dB" ซึ่งเป็นสัญญาณรบกวนล้วนๆ
...
- [ ] **LAW 8:** ค่าที่แก้อยู่ใน `docs/FANOUT.md` ไหม? วัดครบทุกช่องที่มันแตกไปหรือยัง?
- [ ] **LAW 8:** แพทช์นี้เปลี่ยนอย่างเดียวใช่ไหม? มีตัวเลข bench ก่อน/หลัง ในข้อความ commit หรือยัง?
- [ ] **LAW 8:** ผลต่างที่เคลม พ้น `noiseDb` แล้วหรือยัง? (`verdict` ไม่ใช่ `noise`)
