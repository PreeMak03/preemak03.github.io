...
7. **ห้ามเขียนทับ `this._rpm` ตรงๆ — ให้ตั้งเป้าหมายแล้วปล่อยให้ตัวจำกัดอัตราพาไป**
   อาการกระตุกที่เจ้าของบ่นมาเป็นเดือนคือบรรทัดเดียว: ตอนเปลี่ยนเกียร์มีการ
   `this._rpm = ...` ซึ่ง**เป็นการกำหนดค่า ไม่ใช่อัตรา** มันจึงเดินผ่านเพดานทุกตัว
   ปรับ `maxFallStPerSec` เท่าไหร่ก็ไม่ขยับ (ลองสี่ค่า ได้ −1013.5 st/s เท่ากันหมด
   — **ความเท่ากันนั่นแหละคือเบาะแส**)
   ตัวจับถาวรอยู่ใน bench แล้ว: `revStep` = การกระโดดสูงสุดในหนึ่ง tick หน่วย semitone
   **เกิน ~2 st เมื่อไหร่ = มีใครเขียนทับรอบอยู่ ไปหา `this._rpm =` ก่อนจูนอย่างอื่น**
   ค่าอ้างอิง: เทเลพอร์ตเดิม 12.1 st · หลังแก้ 0.7 · classic 18.4 (ยังมี แต่เจ้าของบอกว่าเพราะดี ไม่แตะ)
...
- [ ] **LAW 8:** `revStep` ยังต่ำกว่า 2 st อยู่ไหม? (เกิน = มีการเขียนทับรอบ ไม่ใช่เรื่องจูน)
